# Memory Hierarchy - Overview & Quick Reference
...
- **[Asynchronous Memory Copy (cp.async)](7_async_copy.md)** - **New!** Ampere+ pipelines and asynchronous transfer patterns
...
| **Global Memory** | 300-600 cycles | GPU VRAM | All Threads | Application | Read/Write by all | Main memory for large input/output datasets | [Global Guide](2_global_memory.md) |
...
| **Async Copy** | Optimized | GPU VRAM -> SMEM | Block | Kernel | Direct transfer | Pre-fetching data to shared memory | [Async Copy Guide](7_async_copy.md) |
...
| Latency Hiding                 | Asynchronous Copy (`cp.async`)                      |
...
| **Async Copy Pipeline** | 1.2-1.5x | Compute-bound kernels with memory dependency |
...
Memory Performance Issue?
 Low global memory efficiency?
    Fix coalescing → [Global Guide](2_global_memory.md)
    Use shared memory tiling → [Shared Guide](3_shared_memory.md)
 Shared memory bank conflicts?
    Add padding, fix access patterns → [Shared Guide](3_shared_memory.md#bank-conflicts)
 ...
 Memory Latency Hiding?
    Use Async Copy (`cp.async`) → [Async Copy Guide](7_async_copy.md)
 Still having issues?
     Systematic debugging → [Debug Guide](6_memory_debugging.md)
```
...
4. ** Async Copy**: Use `cp.async` to hide global memory latency on modern GPUs
5. ** Profile Everything**: Use Nsight Compute/Systems to validate optimizations
6. ** Understand Trade-offs**: Sometimes lower occupancy = better performance

## Next Steps

1. **Start with**: [Global Memory Guide](2_global_memory.md) for coalescing basics
2. **Then learn**: [Shared Memory Guide](3_shared_memory.md) for advanced tiling
3. **Specialize with**: [Constant](4_constant_memory.md) or [Unified](5_unified_memory.md) memory guides
4. **Advance to**: [Asynchronous Copy](7_async_copy.md) for modern pipeline optimization
5. **Debug with**: [Debugging Toolkit](6_memory_debugging.md) when performance issues arise
