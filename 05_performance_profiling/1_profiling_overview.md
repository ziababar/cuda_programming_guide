...
| `compute-sanitizer`| Modern runtime memory error checker (Replaces `cuda-memcheck`). Detects **out-of-bounds**, **race conditions**, **invalid memory accesses**. | Validate correctness before optimization; critical for shared/global memory debugging. |
